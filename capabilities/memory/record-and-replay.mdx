---
title: "Record & replay"
description: "Record robot sessions to a SQLite store with memory2, then inspect and query them"
---

This guide covers the **memory2 episode store**: recording a robot session to SQLite (for example with `dimos --replay --replay-db go2_bigoffice run unitree-go2-memory`) and inspecting what was recorded. For the live place memory that powers language navigation, see [Memory](/capabilities/memory).
