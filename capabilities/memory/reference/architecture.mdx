...
| `store/`        | Store ABC + implementations (MemoryStore, SqliteStore) | `dimos/memory2/store/README.md`               |
| `notifier/`     | Notifier ABC + SubjectNotifier                       |                                                  |
| `blobstore/`    | BlobStore ABC + implementations (file, sqlite)       | `dimos/memory2/blobstore/blobstore.md` |
| `codecs/`       | Encode/decode for storage (pickle, JPEG, LCM)        | `dimos/memory2/codecs/README.md`             |
...
| Doc | What |
|-----|------|
| [streaming.mdx](/capabilities/memory/reference/streaming) | Lazy vs materializing vs terminal - evaluation model, live safety |
| [embeddings.mdx](/capabilities/memory/reference/embeddings) | Embedding layer design - EmbeddedObservation, vector search, EmbedImages/EmbedText |
| `dimos/memory2/blobstore/blobstore.md` | BlobStore architecture - separate payload storage from metadata |
