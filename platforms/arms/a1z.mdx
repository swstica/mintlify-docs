...
```bash
# also install the dataset, training, and live-policy runtime (LeRobot)
./dimos/robot/manipulators/galaxea_a1z/scripts/setup_a1z.sh --with-lerobot

# sync and verify Python deps only - do not check or configure CAN hardware
...
DimOS deliberately has **no Linux userspace-CAN fallback**. After a boot or after reconnecting the HHS adapter, rerun the full setup script, or invoke the CAN portion directly. It binds the adapter to the kernel driver, configures the stable `a1zcan` SocketCAN interface, and verifies transmission (it asks for `sudo` only for this part):
...
Select the camera with `--camera-index N` (see [camera access on macOS](#camera-access-macos) above). On a Mac where OpenCV enumerates an external camera as index 0:
...
| Key | Action |
|-----|--------|
| `SPACE` | Save the current episode |
| `g` | Toggle the gripper |
| `d` | Discard the episode in progress |

Pressing `d` while idle discards the most recently saved episode - replay and dataset export then exclude it. The command prints the Memory2 `.db` path under `~/.local/state/dimos/recordings/`.
...
Install (or verify) the complete locked training runtime, then train:

```bash
./dimos/robot/manipulators/galaxea_a1z/scripts/setup_a1z.sh --sdk-only --with-lerobot
```
...
Use `--policy.device=cuda` on an NVIDIA training host, `mps` on Apple silicon, or `cpu` for a slow smoke test. The checkpoint lands at `outputs/a1z_act/checkpoints/last/pretrained_model`.
