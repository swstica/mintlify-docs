---
title: "Quickstart"
description: "Install DimOS, replay a robot session with no hardware, then talk to the robot through an LLM agent."
---

In fifteen minutes you will: install DimOS, replay a recorded Unitree Go2 session in the Rerun viewer with no hardware, and then drive an agentic stack with natural language. Everything on this page runs on a laptop.
...
Extras keep installs lean: `base` is the runtime, modules, transports, and CLI; `unitree` adds WebRTC and skills for Go2 / G1 (real or replayed).
...
<Accordion title="Other install options">
  ...
  Or install system dependencies yourself with the guide for your OS: [Ubuntu](/installation/ubuntu) (primary tested path), [Nix](/installation/nix) (flakes and dev shell), [macOS](/installation/osx) (Homebrew-based, less mature than Linux).
</Accordion>
...
## Step 3: Talk to the robot

Agentic blueprints add an LLM agent that controls the robot by calling skills. The default model is `gpt-4o`, so set your API key first (or see the local-LLM variant below):
...
```bash
dimos agent-send "walk forward one meter, then say hello"
dimos log -f
```

There is also a web chat with voice input at `http://localhost:5555`.

The agent's tools are ordinary MCP skills, and you can call them yourself with no LLM in the loop:
...
No OpenAI key? Run the same stack against a local model with [Ollama](https://ollama.com) (`ollama serve` must be running):

```bash
dimos --simulation run unitree-go2-agentic-ollama
```

How this works, and how to add your own skills: [Agents](/agents).

## Step 4: Real robot

For a physical Unitree Go2 over WebRTC:
