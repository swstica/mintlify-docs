...
Let's use real camera data from the Unitree Go2 robot to demonstrate. We use the [Sensor Storage & Replay](/usage/streams/storage_replay) toolkit, which provides access to recorded robot data:
...
![output](/usage/streams/assets/frame_mosaic2.jpg)

```python skip session=qb output=assets/sharpness_graph2.svg
plot_sharpness(input_frames, sharp_frames, '{output}')
```

![output](/usage/streams/assets/sharpness_graph2.svg)
