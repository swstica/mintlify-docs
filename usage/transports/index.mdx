...
For the general module/stream/blueprint model, see [How DimOS fits together](/usage/how-dimos-fits-together).

Transports connect module streams across process boundaries and/or networks - the mechanics of how a message gets from one module's `Out[T]` to another's `In[T]`. Each edge below is a transported stream (potentially a different protocol per edge); each node is a module:

![go2_nav](/usage/assets/go2_nav.svg)
...
![Benchmark results](/usage/assets/pubsub_benchmark.png)
...
![dimos spy](/usage/assets/lcmspy.png)
...
Add your backend to the same benchmark suite used in [Benchmarks](#benchmarks) above to compare it in context.
